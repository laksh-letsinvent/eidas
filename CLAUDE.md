...
`portal/` is a Next.js app-router static export, live at `eidas.letsinvent.co.uk`. Five nav routes: `/` (the nine-story walkthrough, `components/walkthrough/`), `/atlas` (verbatim), `/experiment` (the brief's Diagnosis + instrument sections via `lib/experiment.ts`'s allowlist, plus the attestation wall and the 13-species catalogue), `/try-it` (four live stories + six live species + a recorded QES run, `components/tryit/`), `/takeaway` (confusion matrix, rate charts, anchor-swap, key learnings, essay links). `/essays`, `/wallet`, `/verify-demo`, `/verify-demo/phish`, `/wallet/present` keep their routes, reachable but unlisted. `lib/experiment.ts`'s `getExperiment()` is an allowlist (only named `##` sections reach the page) rather than a blocklist, specifically so new charter content stays internal by default. Deploy via `portal/deploy/` (Caddy vhost + `deploy.sh`) to `eidas.letsinvent.co.uk`; see `portal/README_DEPLOY.md` (VM host/user intentionally not recorded in this public repo — ask Laksh).
